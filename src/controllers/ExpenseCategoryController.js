...
const ExpenseCategoryController = {
  async createExpenseCategory(req, res) {
    try {
      ...
      return res.status(201).json({ message: 'Catégorie créée avec succès', newCategory });
    } catch (error) {
      console.error('Erreur lors de la création de la catégorie :', error);
      return res.status(500).json({ error: 'Erreur lors de la création de la catégorie', details: error.message });
    }
  },
  ...
  async getAllExpenseCategories(req, res) {
    try {
      ...
      console.error(error);
      return res.status(500).json({ message: 'Erreur lors de la récupération de la catégorie',  error});
    }
  },
  async getExpenseCategorieById(req, res) {
    ...
    if(!categorie){
        return res 
        .status(404).json({ message: 'Catégorie non retrouvée'});
    }
      return res.status(200).json(categorie);
    } catch (error) {
      console.error( error);
      return res.status(500).json({ error: 'Erreur lors de la récupération des catégories', details: error.message });
    }
  },
  ...
  async deleteExpenseCategory(req, res) {
    try {
      ...
      if (isNaN(parsedId)) {
        return res.status(400).json({ message: "ID invalide pour la catégorie de dépense" });
      }
      ...
      return res.status(200).json({ message: 'Catégorie supprimée avec succès', deletedCategory });
    } catch (error) {
      console.error('Erreur lors de la suppression de la catégorie :', error);
      return res.status(500).json({ error: 'Erreur lors de la suppression de la catégorie', details: error.message });
    }
  }
