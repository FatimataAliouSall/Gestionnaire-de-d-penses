...
const PaymentMethodController = {
  ...
  async getAllPaymentMethods(req, res) {
    try {
      ...
      return res.status(500).json({ message: 'Erreur lors de la récupération des modes de paiement', error });
    }
  },
  ...
  async getPaymentMethodById(req, res) {
    try {
      ...
      return res.status(500).json({ error: 'Erreur lors de la récupération du mode de paiement', details: error.message });
    }
  },
  ...
  async deletePaymentMethod(req, res) {
    try {
      ...
      return res.status(200).json({ message: 'Mode de paiement supprimé avec succès', deletedPaymentMethod });
    } catch (error) {
      console.error('Erreur lors de la suppression du mode de paiement :', error);
      return res.status(500).json({ error: 'Erreur lors de la suppression du mode de paiement', details: error.message });
    }
  }
};
