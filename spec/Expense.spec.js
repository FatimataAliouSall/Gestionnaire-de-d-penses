...
describe("Expense Model Tests", () => {
  ...
  beforeAll(() => {
    ...
    spyOn(prisma.expense, "findMany").and.callFake(async () => {
      return [{ id: 1, title: "Rent", amount: 1000.0, frequency: "Monthly", userId: 1 }];
    });
  ...
  it("can be created", async () => {
    const expense = { title: "Rent", amount: 1000.0, frequency: "Monthly", dateCreate: new Date(), startDate: new Date(), endDate: new Date(), userId: 1, expenseCategoryId: 1 };
  ...
  it("can be updated", async () => {
    const updatedExpense = { title: "Electricity", amount: 150.0, frequency: "Monthly" };
    const result = await prisma.expense.update({ where: { id: expenseId }, data: updatedExpense });
  ...
  it("fails to update an expense that does not exist", async () => {
    const updatedExpense = { title: "Non-existent", amount: 200.0, frequency: "Yearly" };
    
    await expectAsync(prisma.expense.update({ where: { id: 9999 }, data: updatedExpense }))
  ...
  it("fails to delete an expense that does not exist", async () => {
    await expectAsync(prisma.expense.delete({ where: { id: 9999 } }))
      .toBeRejectedWithError("Expense not found");
  });
});
